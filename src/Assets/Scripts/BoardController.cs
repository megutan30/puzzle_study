...
public class BoardController : MonoBehaviour
{
    ...
    public bool CheckErase(int chainCount)
    {
        ...
        if(chainCount!=-1)//初期化時は得点計算はしない
        {
            //ボーナス計算
            uint colorNum = 0;
            for(;0<colorBits;colorBits>>=1)//経っているビットの数を数える
            {
                colorNum += (colorBits & 1u);
            }

            uint colorBonus = colorBonusTbl[System.Math.Min(colorNum, colorBonusTbl.Length - 1)];
            uint chainBonus = colorBonusTbl[System.Math.Min(chainCount, chainBonusTbl.Length - 1)];
            uint bonus = System.Math.Max(1, chainBonus + connectBonus + colorBonus);//０の時も1入る
    ...
    public bool Erase()
    {
        ...
        //１から増えてちょっとしたら最大に大きくなったあと小さくなって消える
